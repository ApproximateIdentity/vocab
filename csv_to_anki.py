...
def formatter(dict_reader):
    for row in dict_reader:
        ...
        # Sanity checks
        assert row['type'] in ['adj', 'adv', 'noun', 'verb', 'phrase', 'prep', 'conj', 'pron', 'comp', 'interj', 'sup']
        # Formatting
        for first_lang in ['english', 'czech']:
            ...
            elif row['type'] in ['phrase', 'comp', 'interj', 'sup']:
                yield f"{row[first_lang]};{row[second_lang]}"
            else:
                raise NotImplementedError(f"row type: {row['type']}")

for row in formatter(csv.DictReader(open('news.csv'))):
    print(row)
